...
└── LLM-path-recommendation/    # 基于大语言模型的学习路径推荐系统
```
...
### 4. LLM-path-recommendation（大语言模型学习路径推荐系统）

LLM-path-recommendation是一个基于大语言模型的学习路径推荐系统，用于为学生生成个性化的学习路径，结合AAKT认知诊断结果和知识图谱数据，提供可解释性的学习路径推荐。
...
**项目结构：**
```
LLM-path-recommendation/
├── api/                     # API接口模块
│   ├── ai_service.py        # 大语言模型服务
│   ├── views.py             # API视图
│   └── urls.py              # 路由配置
├── backend/                 # Django项目配置
│   ├── settings.py          # 项目配置
...
**主要功能：**
- **个性化路径推荐**: 根据学生的认知诊断结果和目标知识点，生成个性化学习路径
- **可解释性推荐**: 提供详细的推荐依据和理由
- **知识图谱集成**: 结合Neo4j知识图谱数据，考虑知识点之间的先修关系
- **缓存机制**: 实现多级缓存，提高系统响应速度
- **并行处理**: 采用多线程并行处理，减少API调用时间

**核心文件：**
- `api/ai_service.py`: 大语言模型服务实现，包括路径生成、知识图谱查询等功能
- `api/views.py`: API视图，处理前端请求
- `api/urls.py`: 路由配置，定义API端点

**集成方式：**
LLM推荐系统已集成到IGS-back-end中，通过`llm_recommendation`应用提供服务。前端通过`/llm/recommend/`API接口调用路径推荐功能。
...
5. 通过`llm_recommendation`应用集成LLM推荐系统
6. 提供`/llm/recommend/`API接口供前端调用路径推荐功能
...
5. 后端处理预测结果并返回给前端
6. 前端展示诊断报告和学习推荐
7. 学生选择目标知识点，请求学习路径推荐
8. 前端调用`/llm/recommend/`API接口
9. 后端调用LLM推荐系统，结合AAKT诊断结果和知识图谱数据
10. LLM推荐系统生成个性化学习路径和推荐依据
11. 后端返回路径推荐结果给前端
12. 前端展示学习路径和推荐依据
...
2. 分别配置前端、后端和模型环境
3. 配置LLM推荐系统：
   - 在`IGS-back-end/llm_recommendation/ai_service.py`中配置智谱AI API密钥
   - 确保Neo4j数据库连接正常
4. 启动后端服务：`python manage.py runserver`
...
3. 数据库：配置独立的MySQL服务器
4. 知识图谱：配置独立的Neo4j服务器
5. 模型：确保模型文件正确部署，路径配置正确
6. LLM推荐系统：
   - 配置智谱AI API密钥（建议使用环境变量）
   - 确保缓存机制正常工作
   - 优化并行处理配置
...
5. **LLM API调用失败**: 检查智谱AI API密钥是否正确配置
6. **知识图谱连接失败**: 检查Neo4j数据库连接配置是否正确
7. **路径推荐响应缓慢**: 检查缓存机制是否正常工作，考虑优化并行处理配置
